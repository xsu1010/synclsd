...
- [ ] ▶ **M3** _[hw]_ — HTTPS `GET` to the GitHub Contents API with TLS verification ON. Riskiest piece — isolate and prove it. _(built; **awaiting hardware verification** — needs `cacert.pem` + `token.txt` on SD, boot, confirm a real GitHub API response over TLS)_
